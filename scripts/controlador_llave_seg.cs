    ...
    void Update()
    {
        if (!FlagLlave)
        {
            ...
            if (Input.GetKey(KeyCode.T) || Input.GetButton("Fire1"))
            {
                ...
                carta.text = "Te encuentras en Ia última etapa del juego. Ante ti, reposa un cofre sobre Ia encimera, aguardando ser descubierto. Dentro esconde un tesoro, cuya llave es una palabra secreta. Esta palabra se halla oculta en una nota, escondida en algún rincón de la casa.\n¿Serás capaz de encontrarla a tiempo y desvelar el secreto del cofre?";
            }
        }
    }
